...
class PriorityBranchIterator:
    ...
    def __next__(self):
        ...
        self._priorities.extend(
            p
            for p in blocker.parents
...
DOUBLE_WIDTH = re.compile(
    "[\u1b00-\u1bff\U0001f538\U0001f539]|[\u3dd8][\udc00-\udfff]", re.UNICODE
)
...
def printGraph(clearScreen=False, ciTools=()):
    ...
    for b, row in layoutAllBranches():
        ...
        ciStatuses = [
            status
            for tool in ciTools
            for status in list(tool.ciStatus(b).values())
            if status
        ]
        if ciStatuses:
        ...
        if sys.stdout.isatty():
            ...
            if displayLen(stripEscapeCodes(line)) > columns and len(name) > 10:
                space = max(
                    10, columns + len(name) - displayLen(stripEscapeCodes(line))
                )
                name = b.name
                if "/" in name:
