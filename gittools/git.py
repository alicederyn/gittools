...
class GitLockWatcher(watchdog.events.FileSystemEventHandler):
    ...
    def is_locked(self):
        return (
            self._unlock_timestamp is not None
            and self._unlock_timestamp <= datetime.utcnow()
        )
...
class GitListener(watchdog.events.FileSystemEventHandler):
    ...
    def __init__(self, root_dir=None, exclude_globs=(), include_globs=()):
...
class LazyGitProperty(watchdog.events.FileSystemEventHandler, property):
    ...
    def __init__(self, func, watching):
        ...
        self._watching = frozenset(
            [watching] if isinstance(watching, str) else watching
        )
        update_wrapper(self, func)

    def substitute(self, obj, globs):
        return tuple(
            LazyGitProperty.PROPERTY_RE.sub(lambda m: getattr(obj, m.group(1)), g)
            for g in globs
        )
...
class Branch:
    ...
    def REMOTES():
        ...
        return frozenset(
            Branch(name) for name in names if name.split("/", 1)[-1] in locals
        )
    ...
    def upstreamCommit(self):
        ...
        firstUpstreamReference = first(
            h.hash for h in self.upstream._refLog if h.hash in commitHashes
        )
        ...
        def impl():
            for c in self.allCommits:
                if c == self.upstreamCommit:
                    return
                mergedBranches = [
                    Branch(name) for name in Branch._mergedBranches(c.subject)
                ]
                if mergedBranches:
    ...
    def modtime(self):
        ...
        with Sh(
            "/usr/local/bin/git", "log", "-n5", "--format=%at", self.name, "--"
        ) as log:
            for line in log:
                modtime = int(line)
                if modtime != 1:
